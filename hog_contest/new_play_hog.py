...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if current_player == 0:
            ...
            prev_earned = prev_earned0
            current_lst = lst0
        else:
            current_score, opponent_score = score1, score0
            current_strat = strategy1
            prev_earned = prev_earned1
            current_lst = lst1

        roll_amount = current_strat(current_score, opponent_score)
        points_earned = take_turn(roll_amount, opponent_score, dice)
        current_score += points_earned
        current_list.append([current_score, opponent_score, roll_amount])

        if feral_hogs and abs(roll_amount - prev_earned) == 2:
                current_score += 3
        prev_earned = points_earned

        if is_swap(current_score, opponent_score):
            current_score, opponent_score = opponent_score, current_score

        if current_player == 0:
            score0, score1, prev_earned0 = current_score, opponent_score, prev_earned
        else:
            score0, score1, prev_earned1 = opponent_score, current_score, prev_earned
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 6
    return score0, score1, lst0, lst1
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1):
        new_prev_high = prev_high
        if who == 0:
            current_score = score0
        else:
            current_score = score1
        if (current_score - prev_score) > new_prev_high:
            print(current_score-prev_score, "point(s)! That's the biggest gain yet for Player", who)
            new_prev_high = current_score - prev_score
        return announce_highest(who, new_prev_high, current_score)
    return say
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if not(bad_zero_swap(score, opponent_score)):
        if not(swap_strategy(score, opponent_score, 8)) or not(swap_strategy(score, opponent_score, abs(GOAL_SCORE - score))):
            return 0
        elif score > 89:
